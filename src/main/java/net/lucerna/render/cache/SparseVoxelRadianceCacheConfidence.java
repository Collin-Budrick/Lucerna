...
public record SparseVoxelRadianceCacheConfidence(
        float confidence,
        float variance,
        int sampleCount,
    ...
    public SparseVoxelRadianceCacheConfidence {
    ...
    public static SparseVoxelRadianceCacheConfidence empty(String reason) {
        return new SparseVoxelRadianceCacheConfidence(0.0F, 1.0F, 0, 0L, 0L, false, reason);
    }

    public static SparseVoxelRadianceCacheConfidence dirty(long sourceGeneration, String reason) {
        return new SparseVoxelRadianceCacheConfidence(0.0F, 1.0F, 0, sourceGeneration, 0L, true, reason);
    }

    public static SparseVoxelRadianceCacheConfidence fromSamples(
            int sampleCount,
            float variance,
            long sourceGeneration,
            long lastTouchedFrame,
        ...
        return new SparseVoxelRadianceCacheConfidence(
    ...
    public static SparseVoxelRadianceCacheConfidence merge(Collection<SparseVoxelRadianceCacheConfidence> values) {
        Objects.requireNonNull(values, "values");
        if (values.isEmpty()) {
            return empty("no sparse voxel radiance cache records");
        }
        ...
        for (SparseVoxelRadianceCacheConfidence value : values) {
        ...
        return new SparseVoxelRadianceCacheConfidence(
                ...
                maxTouchedFrame,
                dirty,
                dirty ? "merged dirty sparse voxel radiance cache records" : "merged sparse voxel radiance cache records"
        );
    }
