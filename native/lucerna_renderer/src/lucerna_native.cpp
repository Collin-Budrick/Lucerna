...
lucerna::DirectLightingPayloadPacket make_direct_lighting_payload_packet(
        JNIEnv* env,
        jlong frame_index,
        jlong generation,
        jlong first_generation,
        jlong last_generation,
        jlong celestial_generation,
        jlong emissive_generation,
        jlong shadow_generation,
        jlong shadow_candidate_generation,
        jlong section_snapshot_generation,
        jstring dimension_id,
        jint flags,
        jint celestial_light_count,
        jfloat celestial_light_energy,
        jint selected_emissive_count,
        jfloat selected_emissive_energy,
        jint shadow_candidate_count,
        jint budgeted_shadow_candidate_count,
        jint section_snapshot_count,
        jintArray ray_budget,
        jintArray celestial_light_sources,
        jintArray celestial_light_flags,
        jfloatArray celestial_light_data,
        jobjectArray emissive_light_dimensions,
        jintArray emissive_light_metadata,
        jfloatArray emissive_light_data,
        jlongArray emissive_light_generations,
        jintArray shadow_candidate_metadata,
        jfloatArray shadow_candidate_rays,
        jlongArray shadow_candidate_generations,
        jobjectArray section_snapshot_dimensions,
        jintArray section_snapshot_metadata,
        jlongArray section_snapshot_generations) {
    return lucerna::DirectLightingPayloadPacket{
        to_non_negative_uint64(frame_index, "frameIndex"),
        to_non_negative_uint64(generation, "generation"),
        to_non_negative_uint64(first_generation, "firstGeneration"),
        to_non_negative_uint64(last_generation, "lastGeneration"),
        to_non_negative_uint64(celestial_generation, "celestialGeneration"),
        to_non_negative_uint64(emissive_generation, "emissiveGeneration"),
        to_non_negative_uint64(shadow_generation, "shadowGeneration"),
        to_non_negative_uint64(shadow_candidate_generation, "shadowCandidateGeneration"),
        to_non_negative_uint64(section_snapshot_generation, "sectionSnapshotGeneration"),
        read_string(env, dimension_id, "dimensionId"),
        static_cast<std::uint32_t>(flags),
        celestial_light_count,
        celestial_light_energy,
        selected_emissive_count,
        selected_emissive_energy,
        shadow_candidate_count,
        budgeted_shadow_candidate_count,
        section_snapshot_count,
        read_int_array(env, ray_budget, "directRayBudget"),
        read_int_array(env, celestial_light_sources, "directCelestialLightSources"),
        read_int_array(env, celestial_light_flags, "directCelestialLightFlags"),
        read_float_array(env, celestial_light_data, "directCelestialLightData"),
        read_string_array(env, emissive_light_dimensions, "directEmissiveLightDimensions"),
        read_int_array(env, emissive_light_metadata, "directEmissiveLightMetadata"),
        read_float_array(env, emissive_light_data, "directEmissiveLightData"),
        read_long_array(env, emissive_light_generations, "directEmissiveLightGenerations"),
        read_int_array(env, shadow_candidate_metadata, "directShadowCandidateMetadata"),
        read_float_array(env, shadow_candidate_rays, "directShadowCandidateRays"),
        read_long_array(env, shadow_candidate_generations, "directShadowCandidateGenerations"),
        read_string_array(env, section_snapshot_dimensions, "directSectionSnapshotDimensions"),
        read_int_array(env, section_snapshot_metadata, "directSectionSnapshotMetadata"),
        read_long_array(env, section_snapshot_generations, "directSectionSnapshotGenerations")
    };
}
...
Java_net_lucerna_nativebridge_LucernaNativeBridge_nativeUploadDirectLighting(
        ...
        jlongArray section_snapshot_generations) {
    std::lock_guard lock(g_renderer_mutex);
    return call_initialized_renderer("uploadDirectLighting", [=](lucerna::Renderer& renderer) {
        renderer.upload_direct_lighting_payload(make_direct_lighting_payload_packet(
                env,
                frame_index,
                generation,
                first_generation,
                last_generation,
                celestial_generation,
                emissive_generation,
                shadow_generation,
                shadow_candidate_generation,
                section_snapshot_generation,
                dimension_id,
                flags,
                celestial_light_count,
                celestial_light_energy,
                ...
                section_snapshot_count,
                ray_budget,
                celestial_light_sources,
                celestial_light_flags,
                celestial_light_data,
                emissive_light_dimensions,
                emissive_light_metadata,
                emissive_light_data,
                emissive_light_generations,
                shadow_candidate_metadata,
                shadow_candidate_rays,
                shadow_candidate_generations,
                section_snapshot_dimensions,
                section_snapshot_metadata,
                section_snapshot_generations
        ));
    });
}
...
extern "C" JNIEXPORT jbyteArray JNICALL
Java_net_lucerna_nativebridge_LucernaNativeBridge_nativeDenoisedDiffuseGiCpuOutputPreviewRgba8(JNIEnv* env, jclass) {
    std::lock_guard lock(g_renderer_mutex);
    try {
        const auto rgba8 = initialized_renderer().denoised_diffuse_gi_cpu_output_preview_rgba8();
        constexpr std::size_t kMaxDenoisedDiffuseGiPreviewRgba8Bytes = 1024 * 1024 * 4;
        if (rgba8.size() > kMaxDenoisedDiffuseGiPreviewRgba8Bytes || rgba8.size() % 4 != 0) {
            set_last_error("denoisedDiffuseGiCpuOutputPreviewRgba8", "native denoised diffuse-GI RGBA8 preview payload size is invalid");
            return env->NewByteArray(0);
        }
        if (rgba8.size() > static_cast<std::size_t>(std::numeric_limits<jsize>::max())) {
            set_last_error("denoisedDiffuseGiCpuOutputPreviewRgba8", "native denoised diffuse-GI RGBA8 preview payload exceeds JNI byte array limit");
            return nullptr;
        }
        auto result = env->NewByteArray(static_cast<jsize>(rgba8.size()));
        if (result == nullptr) {
            set_last_error("denoisedDiffuseGiCpuOutputPreviewRgba8", "failed to allocate Java byte array");
            return nullptr;
        }
        if (!rgba8.empty()) {
            env->SetByteArrayRegion(
        ...
        set_last_error("denoisedDiffuseGiCpuOutputPreviewRgba8", exception.what());
    } catch (...) {
        set_last_error("denoisedDiffuseGiCpuOutputPreviewRgba8", "unknown native exception");
    }
    return env->NewByteArray(0);
}
