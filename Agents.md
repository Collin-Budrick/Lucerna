...
Current controller estimate: **about 99.55% complete against this file**.
...
- Latest controller work adds Round 7 signal-separated denoise contracts, final composite mode/status controls, native denoise scaffold telemetry, first-practical CPU denoised diffuse-GI RGBA8 output, controller-selectable baseline/raw-GI/denoised-GI/final modes, a controller-only Round 7 proof helper, native surface-response strengthening, a distinct final raw-plus-denoised composite draw path, auto-selected affected-surface proof crops, and clearer final-mode source telemetry. Build/native/signing and Sodium + Iris + Vulkan world-join launches validate raw-GI metadata, CPU denoised-output generation/readback, composite placeholder metadata, explicit `realDenoiseShaderOutput=false`, HUD-safe selected-mode screenshots, and a passing focused-region raw/denoised/final visual proof.
...
- Real denoise shader/output, edge-preservation proof, and flicker/noise comparison remain open.
...
- Direct-only mode capture and particle/translucency-specific visual proof remain open.
...
Direct-only capture, particle/translucency-specific proof, real shader denoise quality, and temporal/flicker proof remain open.
...
Move beyond the now-validated direct/emissive surface proof by improving the remaining first-lighting stack: low-res diffuse GI should become less selected-mode/debug-like and more physically tied to the scene, denoised GI should show quality improvement instead of only CPU-output/readback proof, and the final composite should blend direct + GI + denoised output as a stable image while preserving HUD/translucency. Keep rejecting metadata-backed preview, focus-window-only brightness, proof markers, and temporary direct-light payload substitution.
