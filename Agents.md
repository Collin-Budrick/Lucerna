...
Current controller estimate: **about 99.965% complete against this file**.
...
- **Partial:** Public Mojang shader visual denoise evidence has log/readiness plumbing and same-scene screenshots, but the latest reviewed image was too subtle and too neutral to count as convincing colored lighting. Native Vulkan offscreen shader-denoise image output remains open.
- Native Vulkan shader denoise evidence: require logs that explicitly report native shader-side denoise dispatch/output image readiness, not CPU denoised readback, public Mojang visual pass, or metadata scaffold status; require same-scene raw-GI versus shader-denoised screenshots, focused affected-surface metrics, edge-preservation checks, and rejection of broad blur/dimming as the only improvement.
